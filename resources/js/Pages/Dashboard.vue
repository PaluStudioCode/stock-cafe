...
const roleLabel = computed(() => labelForValue(summary.value.role));
...
<template>
    <AuthenticatedLayout title="Dasbor">
        <div class="mb-5 flex items-center justify-between gap-3">
            <div>
                <h1 class="text-2xl font-bold">Dasbor {{ roleLabel }}</h1>
            ...
            <button
                ...
                @click="refresh"
            >
                <RefreshCw class="h-4 w-4" :class="{ 'animate-spin': loading }" /> Segarkan
            </button>
        </div>
        ...
        <div class="mt-6 grid gap-5 xl:grid-cols-[1.1fr_.9fr]">
            ...
            <section v-if="summary.role === 'owner'" class="rounded-lg border border-orange-100 bg-white p-4 shadow-sm">
                ...
                <div v-else class="mt-3 space-y-3">
                    <div v-for="activity in summary.recentActivities" :key="activity.id" class="rounded-md bg-stone-50 p-3">
                        ...
                        <div class="text-xs text-slate-500">{{ activity.user?.name || 'Sistem' }} / {{ labelForValue(activity.action) }}</div>
                    </div>
                </div>
            </section>

            <section v-else-if="summary.role === 'inventory_staff'" class="rounded-lg border border-orange-100 bg-white p-4 shadow-sm">
                <h2 class="font-bold">Operasional Inventori</h2>
                <div class="mt-3 space-y-5">
                    <div>
                        <div class="mb-2 text-sm font-semibold text-slate-700">Pesanan Pembelian Terbaru</div>
                        <EmptyState v-if="!summary.recentPurchaseOrders?.length" title="Belum ada pesanan pembelian" />
                        <div v-else class="space-y-2">
                            <div v-for="po in summary.recentPurchaseOrders" :key="po.id" class="flex items-center justify-between rounded-md bg-stone-50 p-3 text-sm">
                                ...
                                <Badge :value="labelForValue(po.status)" />
                            </div>
                        </div>
                    </div>

                    <div>
                        <div class="mb-2 text-sm font-semibold text-slate-700">Pemakaian Stok Terbaru</div>
                        <EmptyState v-if="!summary.recentStockUsages?.length" title="Belum ada pemakaian stok" />
                        <div v-else class="space-y-2">
                            <div v-for="usage in summary.recentStockUsages" :key="usage.id" class="flex items-center justify-between rounded-md bg-stone-50 p-3 text-sm">
                                ...
                                <Badge :value="labelForValue(usage.status)" />
                            </div>
                        </div>
                    </div>

                    <div>
                        <div class="mb-2 text-sm font-semibold text-slate-700">Draf Penyesuaian</div>
                        <EmptyState v-if="!summary.draftAdjustments?.length" title="Tidak ada draf penyesuaian" />
                        <div v-else class="space-y-2">
            ...
            <section v-else class="rounded-lg border border-orange-100 bg-white p-4 shadow-sm">
                <h2 class="font-bold">Catatan Produksi Saya</h2>
                <EmptyState v-if="!summary.productionLogs?.length" class="mt-4" title="Belum ada catatan produksi" message="Catatan produksi yang kamu buat akan tampil di sini." />
                <div v-else class="mt-3 space-y-2">
                    <div v-for="log in summary.productionLogs" :key="log.id" class="flex items-center justify-between rounded-md bg-stone-50 p-3 text-sm">
                        ...
                        <Badge :value="labelForValue(log.status)" />
                    </div>
                </div>
            </section>
        </div>
